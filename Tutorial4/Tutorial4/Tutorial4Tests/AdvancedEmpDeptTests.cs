...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnMinSalaryInDept30()
    {
        ...
        minSalary = emps.Where(e => e.DeptNo == 30)
            .Min(e => e.Sal);
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps
            ...
            .Where(x => x.Sal >= x.Grade * 0 || x.Sal >= grades.First(g => x.Sal >= g.Losal && x.Sal <= g.Hisal).Losal) // lub lepiej:
            .Where(x => grades.Any(g => x.Sal >= g.Losal && x.Sal <= g.Hisal))
            .Select(x => new
            {
                x.EName,
                x.DName,
                Grade = grades.First(g => x.Sal >= g.Losal && x.Sal <= g.Hisal).Grade
            })
            .ToList();
...
// tak samo tutaj, wszystkie odpowiedzi z LINQ ponieważ na wykładzie było że konstrukcja ta jest bardzo podobna w innych językach
// i warto ją umieć 